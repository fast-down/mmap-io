...
mod all_features {
    use mmap_io::{
        create_mmap, MemoryMappedFile, MmapAdvice,
        ChangeEvent,
    };
    use std::fs;
    ...
    fn tmp_path(name: &str) -> PathBuf {
        ...
        p.push(format!("mmap_io_integration_test_{}_{}", name, std::process::id()));
        p
    }
    ...
    fn test_all_features_integration() {
        ...
        // Test advise
        mmap.advise(0, 4096, MmapAdvice::Sequential).expect("advise sequential");
        mmap.advise(4096, 4096, MmapAdvice::Random).expect("advise random");
        ...
        mmap.chunks_mut(1024).for_each_mut(|offset, chunk| {
            ...
            chunk.fill(value);
            Ok::<(), std::io::Error>(())
        }).expect("chunks_mut").expect("for_each_mut");
        ...
        let _handle = mmap.watch(move |_event: ChangeEvent| {
    ...
    fn test_cow_mode_integration() {
        ...
        // Test advise on COW
        cow_mmap.advise(0, 4096, MmapAdvice::WillNeed).expect("advise cow");
        
        // Test iterator on COW
        let pages: Vec<_> = cow_mmap.pages()
            .collect::<Result<Vec<_>, _>>()
    ...
    fn test_concurrent_features() {
        ...
        let handles: Vec<_> = (0..4).map(|thread_id| {
            ...
            thread::spawn(move || {
                ...
                mmap.advise(offset, 2048, MmapAdvice::Sequential).expect("thread advise");
    ...
    fn test_page_aligned_operations() {
        ...
        mmap.advise(0, ps as u64, MmapAdvice::Sequential).expect("advise page 0");
        mmap.advise(ps as u64, ps as u64, MmapAdvice::Random).expect("advise page 1");
        mmap.advise((ps * 2) as u64, ps as u64, MmapAdvice::WillNeed).expect("advise page 2");
        mmap.advise((ps * 3) as u64, ps as u64, MmapAdvice::DontNeed).expect("advise page 3");
...
fn test_iterator_only() {
    ...
    let mmap = create_mmap(path, 4096).expect("create");
    let _chunks: Vec<_> = mmap.chunks(1024)
        .collect::<Result<Vec<_>, _>>()
        .expect("chunks");
